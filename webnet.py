...
class LSTMWebNet(nn.Module):
    ...
    def __init__(self, n_input_nodes:int, n_hidden_nodes:int, n_output_nodes:int, #h_has_self_weights=False, 
        ...
        if all_share_lstm: 
            self.lstm = nn.LSTM(1, cell_state_size, proj_size=1)
        else: 
            if h_share_lstm: 
                self.h_lstm = nn.LSTM(1, h_cell_state_size, proj_size=1) 
            else: 
                self.hs = [nn.LSTM(1, h_cell_state_size, proj_size=1) for _ in range(n_hidden_nodes)] 
            
            if o_share_lstm: 
                self.o_lstm = nn.LSTM(1, o_cell_state_size, proj_size=1)
            else: 
                self.os = [nn.LSTM(1, o_cell_state_size, proj_size=1) for _ in range(n_output_nodes)] 
